...
import { analyticsApi, workflowApi, unwrapList } from '@/api'
...
function StatCard({ title, value, icon: Icon, color, link, subtitle }: {
  ...
  const card = (
    <div className={`panel p-5 flex items-start gap-4 ${link ? 'panel-interactive cursor-pointer' : ''}`}>
      <div className={`w-12 h-12 rounded-registry flex items-center justify-center shrink-0 ${color}`}>
...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setStats(s.data)
      setByType(t.data.map((d: { documentType?: string; document_type?: string; count: number }) => ({
        name: (d.documentType ?? d.document_type ?? '').replace('_', ' '),
        count: d.count,
      })))
      setByMonth(m.data.map(d => ({ month: d.month, count: d.count })))
      setMyTasks(unwrapList(tasks.data))
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[...Array(7)].map((_, i) => <div key={i} className="h-24 skeleton" />)}
        </div>
      </div>
    )
  }
